...
void init()
{
	...
	generateEllipsePoints(circle_vertices, circle_colors, 0, CIRCLE_NUM_POINTS, glm::vec2(0.60, 0.70), 0.25, 1.0);
	generateEllipsePoints(ellipse_vertices, ellipse_colors, 0, ELLIPSE_NUM_POINTS, glm::vec2(-0.60, 0.70), 0.25, 0.5);
...
void display(void)
{
	...
	// @TODO: 绘制圆
	// @TODO: 绘制椭圆
	glBindVertexArray(vao[2]); // 绑定椭圆的VAO
	glDrawArrays(GL_TRIANGLE_FAN, 0, CIRCLE_NUM_POINTS);

	glBindVertexArray(vao[3]); // 绑定圆形的VAO
...
int main(int argc, char** argv)
{
	...
	// 配置窗口属性
	GLFWwindow* window = glfwCreateWindow(512, 512, "2021150261_罗永亮_实验一", NULL, NULL);
	if (window == NULL)
	{
